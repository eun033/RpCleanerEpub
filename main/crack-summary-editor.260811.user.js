...
// @version      1.2.1
// @description  크랙의 장기 요약 메모리를 한꺼번에 편집하고 관리합니다. (version 관리방식: 크랙UI변경.기능추가및수정.핫픽스) - UI 변경
// @author       gemini
// @match        https://crack.wrtn.ai/*
// @updateURL    https://github.com/eun033/RpCleanerEpub/raw/refs/heads/main/main/crack-summary-editor.260811.user.js
// @downloadURL  https://github.com/eun033/RpCleanerEpub/raw/refs/heads/main/main/crack-summary-editor.260811.user.js
// @grant        none
...
(function() {
   ...
   // --- 버튼 주입 ---
    ...
    function injectButton() {
        // 이미 버튼이 존재하면 중복 생성하지 않음
        if (document.getElementById('summary-editor-btn')) return;

        // 상단 우측 '하이퍼챗 1.5' 선택창 또는 관련 드롭다운 컨테이너 탐색
        const hyperChatBtn = Array.from(document.querySelectorAll('button')).find(b => b.textContent.includes('하이퍼챗'));
        const targetContainer = hyperChatBtn?.parentElement || document.querySelector('.flex.items-center.gap-2');
        ...
        // 상단 헤더 디자인에 이질감 없이 맞춰지는 스타일링
        btn.style.cssText = 'padding: 4px 10px; height: 32px; background: #2563eb; color: white; border-radius: 6px; font-weight: 600; font-size: 13px; border: none; cursor: pointer; white-space: nowrap; margin-right: 6px; z-index: 9999; display: inline-flex; align-items: center; justify-content: center;';
        ...
        // '하이퍼챗 1.5' 버튼 바로 앞(왼쪽)에 주입
        if (hyperChatBtn) {
            targetContainer.insertBefore(btn, hyperChatBtn);
        } else {
